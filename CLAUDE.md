...
- **Proxy compression (v7.9.39).** `netlify/functions/sb.js` reads `SB_GZIP` from the Netlify environment: unset/`0` = plain JSON both legs (default); `1` = ask the gateway for gzip and inflate in the function (shrinks the cross-Atlantic leg, browser unchanged); `passthrough` = hand the compressed bytes to the browser (`isBase64Encoded` + `Content-Encoding`). Pass-through was the v7.9.38 default and broke Dev (every read failed and retried), so enable a mode only after checking one `/sb/` response in devtools on Dev. `system_users` always stays plain because its body is scrubbed.
