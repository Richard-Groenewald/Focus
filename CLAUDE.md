...
- **Proxy compression (v7.9.38).** `netlify/functions/sb.js` asks the gateway for gzip and hands the compressed bytes to the browser (`isBase64Encoded`); `SB_GZIP=0` in the Netlify environment switches it off without a deploy. `system_users` stays plain because its body is scrubbed.
