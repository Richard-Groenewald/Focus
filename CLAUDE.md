...
- **Uniforms (v7.9.56).** `quote_accessories.category` gains `uniform` (`sql/uniforms.sql` widens the check and seeds Basic / Combat / Executive/Corporate / Specialised at the quote tool's 2025-04-23 prices, per officer, amortised). `pbItemPerPost(item)` (workforce or uniform) is the test every per-post category check uses — the workforce rollup, the Contract tab filter and `pbSupportEval`'s quote-level test go through it. `pbChecklistCols` renders a third column, Uniform, as radios (`pbPickUniform(postId, id|0)`): the pick DELETEs any other uniform tick on the post, POSTs the new tick and PATCHes `quote_posts.uniform` with the name in one round; an auto-ticked uniform must be overridden off (reason row) before another is picked. Uniform items are hidden from the Workforce column and from the proposal's workforce list; `pbPostUniformName(p)` (ticked uniform, else legacy `p.uniform`) feeds the scope table's Uniform column. The rule editor offers uniforms as triggers only (`_supItemGroups(titles, true)`). Without the SQL the category is rejected on add (`pbItemAdd` names the file) and the Uniform column shows its empty hint.
- **Training prices (v7.9.56).** The Xone quote tool prices training as a monthly provision per officer (startup 0, useful life 1 month). That maps onto the unchanged training model as `quote_training_costs.cost_per_officer` = the monthly figure with `validity_months = 1`; `pbItemCostLabel` and the admin cost history read a 1-month cycle as "per officer per month" / "monthly". `sql/training_costs_seed.sql` is the idempotent baseline (courses matched by code, in-use date 2024-01-01). Training is still not priced by the builder — the calc engine should treat cycle 1 as cost ÷ 1 per month and a longer cycle as cost ÷ cycle.
- **Support items baseline (v7.9.56).** `sql/support_items_seed.sql` makes the non-uniform catalogue exactly the quote tool's 20 items at their current price: listed items are upserted by `code` (ids kept so ticks and conditions survive), unlisted non-uniform items are removed with their ticks, adjustments, condition members and costs, and every old cost row on the listed items is replaced. The quote tool has no quantity basis, so the script assigns one (personal kit per officer, shared post kit per duty position, occurrence book per post, bullet trap and gun safe per contract) — a different rule is a new cost row from a date in Site Admin. "Active = No" maps to `active = false`.
- **Contract and service items baseline (v7.9.56).** `sql/contract_service_items_seed.sql` does the same for the quote tool's contract-level list (40 items: office kit, radios, monitoring, cellphones, dogs, cleaning, guard hut, fire extinguishers, Instacom subscriptions, Halo radio), all basis `per_contract`; physical things are `contract`, monthly services `service`. Cost mapping: life > 0 → amortised; startup with life 0 → `once_off` (Medical Kit); startup 0 → monthly only, life not stored. Bullet trap and site gun safe stay with `sql/support_items_seed.sql`. Polygraphs is captured as 800.00 over a 1-month life (R800/month per contract) — flagged as likely meant once-off per officer. The mock seed does not carry this list.
