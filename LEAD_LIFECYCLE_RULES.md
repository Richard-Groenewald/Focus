...
| **Promoted** | Became an opportunity | SM approves at Review (existing promotion machinery) | Terminal |
...
e.g. "Qualified: 14 (Review 5 · Nurture 6 · Hold 2 · Promoted 1)" — so the number cannot be
...
special-request path is superseded by the auto-flip to Review.
...
| Review | **FACT all green** + service category + organisation |
...
| Working/Review → Nurture | Owner or SM, **direct action**; F/A green; cadence mandatory | None |
| Hold → Working | Wake date expires | None (auto) — owner + SM get top-of-dashboard reminders |
| Hold → Review | The full Review gate is met while on Hold (FACT re-greens + service + organisation) | None (auto) — wake date cleared, owner notified |
| Hold → Working/Review | **Early release** — owner or SM takes it off Hold before the wake date | None (direct) — wake date cleared, landing stage derived off the data |
| Nurture → Working/Review | **Exit** — owner or SM releases it from Nurture | None (direct) — cadence cleared, landing stage derived off the data |
...
no approval, no `lead_stage_requests` row. The engine derives the landing stage: Review if the
full Review gate is met, otherwise Working.

**Organisation requirement sits at the Review gate** (2026-08-08, reconfirmed 2026-08-12): an
organisation identity (linked record or typed name) is not part of Working completeness — a
lead can be worked without knowing whose site it is. It is mandatory at Review, and therefore
for promotion. Once FACT goes all-green, any field still blocking Review (service interest,
...
- No data migration for the rename — `Qualified` stays the stored value; **Review** is a display
  label only.
...
- Organisation-required-for-Working (moved to the Qualified/Review gate, 2026-08-08)
