...
> **Status (v7.9.36, 9 September 2026):** Tier 0 is implemented on branch `claude/focus-performance-optimization-3tpv6y`. Items 0.1–0.9, 0.11–0.16 are done as described; 0.10 covers the client-contacts embed, the eligible-people embed and the Engagement History chunk loops (the engagement-modal opener was left as is). Every database-facing change carries a fallback to the previous code path, so a database without a relationship or constraint degrades to the old behaviour instead of failing. **Tier 1 (v7.9.37):** the SQL is in `sql/perf_tier1_indexes.sql` (reconciled with the July `add_perf_indexes.sql` on `test`), `sql/perf_tier1_triggers.sql`, `sql/perf_tier1_audit_statement_level.sql` and `sql/perf_tier1_sweeps_heartbeat.sql`, to be run in that order on Dev then prod; the client calls `sweep_leads()` and `heartbeat()` when present and falls back otherwise, and sends a new stream's label in the engagement insert so the `engagements_default_stream` trigger removes the second write. Item 1.6 (PostgREST max rows 1000 → 5000) is a Supabase dashboard setting. **Tier 2 (v7.9.38, first objects):** `sql/perf_tier2_dashboard_summary.sql` (2.3 — `dashboard_summary(p_from)`, one jsonb bundle for all thirteen widgets and the Attention Workbench; ~30 requests → 1, scope switches client-only) and `sql/perf_tier2_engagements_labelled.sql` (2.6 — the labelled view; Engagement History 3 rounds → 1), both with fallbacks to the old reads; 3.2 (gzip through the proxy) shipped in v7.9.40 as a client-negotiated private header (`X-Focus-Gzip` / `X-Focus-Encoding`, inflated by `DecompressionStream` in the browser), because Netlify does not compress function responses and a `Content-Encoding` pass-through could not be verified through its edge. Both shapes were proved equivalent to the old code paths over a Dev snapshot (`tests/perf/`). Still proposals: 2.1/2.2, 2.4, 2.5, 2.7, 2.8, 2.10–2.12, 3.3–3.5, Tier 4.
